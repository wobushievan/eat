import { calculateAll } from '../../services/calc/result'
import { getLatestForm, saveLatestForm } from '../../services/storage/form'
import { setCurrentResultPayload } from '../../store/session'
import type {
  ActivityLevel,
  BodyInputMode,
  BodyType,
...
function findProvinceIndex(value: ProvinceCode): number {
...
function buildRatioFromBoundaries(first: number, second: number): RatioValue {
...
function buildBoundariesFromRatio(ratio: RatioValue): RatioBoundaries {
  return {
...
function normalizeRatioValue(ratio: RatioValue): RatioValue {
...
function validateAll(formData: UserFormData, today: string): string {
  ...
  if (formData.birthDate < TODAY_MIN) {
...
Page({
  ...
  onSexChange(event: WechatMiniprogram.CustomEvent<{ value: Sex }>) {
    this.setData({
  ...
  onBodyModeChange(event: WechatMiniprogram.CustomEvent<{ value: BodyInputMode }>) {
  ...
  onBodyTypeChange(event: WechatMiniprogram.CustomEvent<{ value: BodyType }>) {
    this.setData({
  ...
  onActivityChange(event: WechatMiniprogram.CustomEvent<{ value: ActivityLevel }>) {
    this.setData({
  ...
  onChildhoodProvinceChange(event: WechatMiniprogram.PickerChange) {
  ...
  onAdultProvinceChange(event: WechatMiniprogram.PickerChange) {
    const index = Number(event.detail.value)
    const province = PROVINCE_OPTIONS[index]
  ...
  applyRatioBoundaries(first: number, second: number) {
  ...
  updateRatioByPageX(pageX: number, handle: 'first' | 'second') {
  ...
  onRatioHandleStart(event: WechatMiniprogram.TouchEvent) {
    const handle = event.currentTarget.dataset.handle as 'first' | 'second'
  ...
  onRatioHandleMove(event: WechatMiniprogram.TouchEvent) {
    const handle = (this.data.activeRatioHandle || event.currentTarget.dataset.handle) as 'first' | 'second'
    const touch = event.touches[0]
  ...
  onRatioTrackTap(event: WechatMiniprogram.TouchEvent) {
  ...
  onTasteChange(event: WechatMiniprogram.CustomEvent<{ value: TasteLevel }>) {
    this.setData({
  ...
  onEatOutChange(event: WechatMiniprogram.CustomEvent<{ value: EatOutLevel }>) {
    this.setData({
