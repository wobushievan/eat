...
function buildStepState(currentStep) {
  return {
    currentStep,
...
function validateStep1(formData, today) {
...
function validateStep2(formData) {
  ...
  if (!formData.bodyType) return '选一下你觉得自己的体型'
  return ''
}
...
function validateCurrentStep(formData, today, step) {
  if (step === 1) return validateStep1(formData, today)
  if (step === 2) return validateStep2(formData)
...
Page({
  ...
  handleNext() {
    ...
    if (this.data.currentStep >= TOTAL_STEPS) return
